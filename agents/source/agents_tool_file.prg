...
FUNCTION AGTOOL_Read()
   RETURN { "name" => "read", ;
            ...
            "parameters" => { "type" => "object", ;
               ...
               "required" => { "path" } }, ;
            "handler" => {| hArgs | AGTOOL_ReadRun( hArgs ) } }

STATIC FUNCTION AGTOOL_ReadRun( hArgs )
...
FUNCTION AGTOOL_Write()
   RETURN { "name" => "write", ;
            ...
            "handler" => {| hArgs | AGTOOL_WriteRun( hArgs ) } }

STATIC FUNCTION AGTOOL_WriteRun( hArgs )
...
FUNCTION AGTOOL_Edit()
   RETURN { "name" => "edit", ;
            ...
            "handler" => {| hArgs | AGTOOL_EditRun( hArgs ) } }

STATIC FUNCTION AGTOOL_EditRun( hArgs )
   ...
   cText   := cBefore
   nCount  := AGTOOL_CountSub( cText, cOld )
   IF nCount == 0
   ...
   IF !hb_MemoWrit( cPath, cText )
      RETURN "Error: cannot write " + cPath
   ENDIF
   RETURN AGDIFF_Lines( cBefore, cText )

// Counts non-overlapping occurrences of cSub in cText.
STATIC FUNCTION AGTOOL_CountSub( cText, cSub )
