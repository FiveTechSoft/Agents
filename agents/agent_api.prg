// Agents API — Streaming SSE parser and curl-based HTTP transport.
// Refactored from CCHarbour's ccsse.prg, cchttp.prg, and ccapi.prg.
// All functions are module-level; called by Agent:Step().

#include "fileio.ch"

// ============================================================================
// SSE Parser (ported from ccsse.prg)
// ============================================================================

// Creates a fresh SSE parser state.
FUNCTION AGENT_SSE_New()
RETURN { "buffer" => "", "closed" => .F. }

// Feeds raw bytes to the SSE parser. Calls bEmit per parsed event.
...
// Parses one SSE line (after "data:" prefix).
...
// Emits tool_call_delta events for each tool call in the delta.
STATIC FUNCTION AGENT_SSE_ToolCalls( aCalls, bEmit )
   ...
   FOR EACH hCall IN aCalls
      ...
      IF hb_HHasKey( hCall, "id" )
         hEv[ "id" ] := hCall[ "id" ]
      ENDIF
      IF hb_HHasKey( hCall, "function" ) .AND. ;
         ValType( hCall[ "function" ] ) == "H"
         hFn := hCall[ "function" ]
...
// Safe emit: only calls bEmit when it's a valid codeblock.
...
// ============================================================================
// Stream accumulator (merges SSE events into a result state)
// ============================================================================

// Feeds a raw text chunk into the SSE parser and updates hState.
...
// Folds one parsed SSE event into hState and forwards to caller.
FUNCTION AGENT_OnEvent( hEv, hState, bOnEvent )
   ...
   CASE hEv[ "type" ] == "tool_call_delta"
      AGENT_AccTool( hState[ "tools" ], hEv )
...
// ============================================================================
// HTTP Transport (curl subprocess — ported from cchttp.prg)
// ============================================================================

// Performs a streaming POST via curl subprocess.
// hReq: { url, headers (array of "K: V"), body, timeout }.
// bOnChunk: called with each raw text chunk received.
...
FUNCTION AGENT_HTTP_Post( hReq, bOnChunk )
   ...
   // unique header dump file (thread-safe)
   ...
   // feed request body to curl's stdin
   ...
   DO WHILE ( nRead := FRead( hOut, @cBuf, hb_BLen( cBuf ) ) ) > 0
      ...
      // Ctrl+C check — use InKey() for console apps
      IF InKey( 0.01 ) == 3   // Ctrl+C
   ...
   // parse HTTP status from header dump
   nStatus := AGENT_HTTP_ParseStatus( cHdrFile )
   FErase( cHdrFile )
...
// Parses the HTTP status code from curl's -D header dump file.
...
// Performs a non-streaming HTTP GET/POST request via curl.
// hReq: { url, method ("GET"/"POST"), headers (array), body, timeout }.
...
// ============================================================================
// UTF-8 Sanitization (ported from cctools.prg)
// ============================================================================

// Replaces invalid UTF-8 byte sequences with "?".
