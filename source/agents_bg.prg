...
FUNCTION AGBG_NextId()
   LOCAL cId
   hb_mutexLock( AGBG_Lock() )
   s_nNextId++
   cId := "bg" + LTrim( Str( s_nNextId ) )
   hb_mutexUnlock( AGBG_Lock() )
   RETURN cId

// Registers a new task. cId comes from AGBG_NextId; the caller is the
...
// cancel_requested) for AGBG_Update.
FUNCTION AGBG_Add( cId, cType, cPrompt, nTimeout )
   LOCAL hTask
   hTask := { "id"               => cId, ;
              "type"             => hb_CStr( cType ), ;
...
FUNCTION AGBG_Update( cId, hPatch )
   LOCAL hTask, cKey
   IF ValType( hPatch ) != "H"
      RETURN NIL
   ENDIF
...
FUNCTION AGBG_CancelRequested( cId )
   LOCAL lFlag := .F.
   hb_mutexLock( AGBG_Lock() )
   IF hb_HHasKey( s_hTasks, cId )
