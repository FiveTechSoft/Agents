...
FUNCTION AGTOOL_AskUser()
   RETURN { "name" => "ask_user", ;
            ...
            "handler" => {| hArgs | AGTOOL_AskUserRun( hArgs ) } }

STATIC FUNCTION AGTOOL_AskUserRun( hArgs )
   LOCAL oSel, cAnswer
   IF ValType( hArgs[ "options" ] ) != "A" .OR. Len( hArgs[ "options" ] ) < 2
      RETURN "Error: 'options' must be an array of at least 2 strings"
   ENDIF
   oSel := AGSEL_New( hb_CStr( hArgs[ "question" ] ), hArgs[ "options" ] )
   cAnswer := AGSEL_Run( oSel )
