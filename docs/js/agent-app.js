...
async function shOne(cmd, stdin){
  ...
  switch(name){
    ...
    case 'toolreg': {
      if(a0.length<2){ SH_EXIT=1; return 'toolreg: '+T('uso: toolreg <nombre> <script>','usage: toolreg <name> <script>'); }
      const nm=(a0[0]||'').toLowerCase().replace(/[^a-z0-9_]/g,'_');
      const pp=shResolve(a0[1]);
      const ff=await fsGet(pp);
      if(!ff){ SH_EXIT=1; return 'toolreg: '+T('script no encontrado: ','script not found: ')+a0[1]; }
      USER_TOOLS.set(nm,{name:nm,desc:a0.slice(2).join(' ')||'(sin descripción)',path:a0[1],type:a0[1].endsWith('.py')?'python':'shell'});
      saveUserTools();
      return 'Tool '+nm+' registrada ('+USER_TOOLS.get(nm).type+').';
    }
    case 'tools': {
...
function saveUserTools(){ try{ localStorage.setItem('user_tools',JSON.stringify([...USER_TOOLS])); }catch(e){} }
let xtermReady = null;
...
/* =====================================================================
   AI Disk Classifier — transformers.js zero-shot (Xenova/mobilebert-xnli).
   Auto-tags every file on the disk: codigo, test, config, docs, etc.
   Lazy-loaded on first use (~100MB ONNX model, cached by browser).
   ===================================================================== */
let classifierReady = null;
async function ensureClassifier(){
  if(classifierReady) return classifierReady;
  tool('🤖 Cargando clasificador IA (transformers.js ~100MB, primera vez)…');
  classifierReady = (async()=>{
    ...
    return await pipeline('zero-shot-classification', 'Xenova/mobilebert-xnli');
  })();
  try{ return await classifierReady; }catch(e){ classifierReady=null; throw e; }
}
const DISK_LABELS=['código','test','configuración','documentación','datos','script shell','otro'];
async function classifyDisk(){
  const classifier = await ensureClassifier();
  const items = await fsList();
  const files = items.filter(f => !f.path.endsWith('/.keep') && !f.path.startsWith('_labels/') && !f.path.startsWith('skills/'));
  if(!files.length) return 'Disco vacío.';
  const results = [];
  for(const f of files){
    const text = (f.content||'').slice(0, 2000); // first 2KB is enough for classification
    if(!text.trim()){ results.push({path:f.path,label:'vacío',score:1}); continue; }
    const out = await classifier(text, DISK_LABELS);
    const label = out.labels[0];
    const score = out.scores[0];
    results.push({path:f.path, label, score});
    // persist label
    await fsPut('_labels/'+f.path+'.label', JSON.stringify({label,score,at:Date.now()}));
  }
  return results;
}
async function classifyCard(){
  tool('🤖 Clasificando archivos del disco…');
  const results = await classifyDisk();
  if(typeof results==='string'){ tool(results); return; }
  const c=el('<div class="bg-gray-800 border border-purple-900/50 rounded-xl overflow-hidden max-w-[90%]"></div>');
  c.innerHTML='<div class="p-3 border-b border-gray-700 flex items-center gap-2"><span class="text-purple-400 text-lg">🤖</span><div><h4 class="text-sm font-semibold text-gray-200">Disco Clasificado</h4><p class="text-[10px] text-gray-400">'+results.length+' archivos etiquetados con IA</p></div></div><div class="p-3 bg-gray-900/50 space-y-1.5"></div>';
  const body=c.querySelector('.p-3.bg-gray-900\\/50');
  results.sort((a,b)=>a.label.localeCompare(b.label)||b.score-a.score);
  results.forEach(r=>{
    const row=el('<div class="flex items-center justify-between text-xs"></div>');
    const colormap={código:'text-blue-400',test:'text-green-400',configuración:'text-yellow-400',documentación:'text-gray-400',datos:'text-orange-400','script shell':'text-cyan-400',otro:'text-gray-500',vacío:'text-gray-600'};
    row.innerHTML='<span class="truncate flex-1">📄 '+r.path+'</span><span class="'+((colormap[r.label])||'text-gray-500')+' bg-gray-800 px-1.5 py-0.5 rounded text-[10px] shrink-0 ml-2">'+r.label+' '+(r.score>0.5?'✅':'🤔')+'</span>';
...
function bot(html,silent){ const d=el('<div class="bg-gray-800 border border-gray-700 p-3 rounded-xl rounded-tl-none max-w-[90%]"></div>'); d.innerHTML=html; chat().appendChild(d); down();
  if(TTS_ON && !silent) speak(mdToText(html));
  return d; }

/* ===== Voz: lectura de respuestas (speechSynthesis) + dictado (SpeechRecognition) ===== */
let TTS_ON=(()=>{ try{ return localStorage.getItem('tts')==='1'; }catch(e){ return false; } })();
const TTS_LOCALES={es:'es-ES',en:'en-US',fr:'fr-FR',de:'de-DE',pt:'pt-PT',it:'it-IT'};
function mdToText(html){ const d=document.createElement('div'); d.innerHTML=String(html).replace(/<code>/g,' ').replace(/<\/code>/g,' ');
  return (d.textContent||'').replace(/```[\s\S]*?```/g,' (código omitido) ').replace(/\s+/g,' ').trim(); }
function speak(text){
  if(!TTS_ON || !text || !window.speechSynthesis) return;
  try{
...
function toggleTts(){
  ...
  if(!TTS_ON && window.speechSynthesis){ try{ speechSynthesis.cancel(); }catch(e){} }
...
function toggleMic(){
  ...
  if(!SR){ tool(T('Tu navegador no soporta dictado (SpeechRecognition). Prueba Chrome o Edge.','Your browser does not support speech recognition. Try Chrome or Edge.')); return; }
  if(REC_ON){ try{ REC.stop(); }catch(e){} return; }
  try{
    REC=new SR(); REC.lang=TTS_LOCALES[curLang]||'es-ES'; REC.interimResults=true; REC.continuous=false;
    const i=document.getElementById('prompt'); const base=i.value;
    REC.onresult=e=>{ let t=''; for(const r of e.results) t+=r[0].transcript; i.value=(base?base+' ':'')+t; acUpdate(); };
    REC.onend=()=>{ REC_ON=false; const b=document.getElementById('micbtn'); if(b) b.classList.remove('bg-red-600','animate-pulse');
      const v=i.value.trim(); if(v && !v.startsWith('/')) runCmd(v); };   // auto-send the dictation
    REC.onerror=e=>{ tool('🎤 '+e.error); };
    REC.start(); REC_ON=true;
    const b=document.getElementById('micbtn'); if(b) b.classList.add('bg-red-600','animate-pulse');
    tool(T('🎤 Escuchando… habla y se enviará al parar.','🎤 Listening… speak and it will be sent when you stop.'));
  }catch(e){ tool('🎤 error: '+e); REC_ON=false; }
}
...
/* ===== /action — GitHub Actions desde el agente (list | run | runs) ===== */
function actionRepo(){ return GIT_REPO || document.getElementById('ghrepo').value.trim() || 'FiveTechSoft/Agents'; }
async function actionCmd(arg){
  const sp=(arg||'').indexOf(' ');
  const sub=(sp<0?(arg||''):arg.slice(0,sp)).toLowerCase();
  const rest=sp<0?'':arg.slice(sp+1).trim();
  const repo=actionRepo(), H=ghHeaders();
  if(sub==='list'||sub===''){
    setWorking(true);
    try{
      const r=await fetch('https://api.github.com/repos/'+repo+'/actions/workflows',{headers:H});
      const j=await r.json();
      if(!r.ok){ tool('/action: HTTP '+r.status+' ('+((j.message)||repo)+')'); return; }
      const wfs=(j.workflows||[]).map(w=>w.path.split('/').pop()+'  ·  '+w.name+'  ['+w.state+']');
      tool(wfs.length? ('Workflows en '+repo+':\n'+wfs.join('\n')) : '/action: sin workflows en '+repo);
    }catch(e){ tool('/action error: '+e); } finally{ setWorking(false); }
    return;
  }
  if(sub==='runs'){
    setWorking(true);
    try{
      const r=await fetch('https://api.github.com/repos/'+repo+'/actions/runs?per_page=8',{headers:H});
      const j=await r.json();
      if(!r.ok){ tool('/action: HTTP '+r.status); return; }
      const rows=(j.workflow_runs||[]).map(x=>x.run_number+' · '+x.name+' · '+x.head_branch+' · '+
        (x.conclusion||x.status)+(x.html_url?(' \u2192 '+x.html_url):''));
      tool(rows.length? ('Últimos runs en '+repo+':\n'+rows.join('\n')) : '/action: sin runs.');
    }catch(e){ tool('/action error: '+e); } finally{ setWorking(false); }
    return;
  }
  if(sub==='run'){
    const parts=rest.split(/\s+/); const wf=parts[0]; const ref=parts[1]||(await actionDefaultBranch(repo,H));
    if(!wf){ tool('Uso: /action run <workflow.yml|nombre> [rama]'); return; }
    setWorking(true);
    try{
      // resolve id by filename or display name
      const lw=await (await fetch('https://api.github.com/repos/'+repo+'/actions/workflows',{headers:H})).json();
      const w=(lw.workflows||[]).find(w=>w.path.split('/').pop().toLowerCase()===wf.toLowerCase()||w.name.toLowerCase()===wf.toLowerCase()||w.id==wf);
      if(!w){ tool('/action: workflow no encontrado: '+wf+'. Usa /action list.'); return; }
      const dr=await fetch('https://api.github.com/repos/'+repo+'/actions/workflows/'+w.id+'/dispatches',
        {method:'POST',headers:Object.assign({'Content-Type':'application/json'},H),body:JSON.stringify({ref})});
      if(dr.status!==204){ tool('/action dispatch: HTTP '+dr.status+(dr.status===403?' (el token necesita permiso actions:write)':'')); return; }
      tool('▶ Dispatch '+w.path.split('/').pop()+' en '+ref+' …');
      await actionWaitRun(repo,w.name,ref,H);
    }catch(e){ tool('/action error: '+e); } finally{ setWorking(false); }
    return;
  }
  if(sub==='setup'){
    // real execution in the cloud: a dispatchable workflow that runs any shell command
    const yml='name: Agents Task\n'+
      'on:\n  workflow_dispatch:\n    inputs:\n      cmd:\n        description: "Shell command to run"\n        required: true\n'+
      'jobs:\n  run:\n    runs-on: ubuntu-latest\n    steps:\n'+
      '      - uses: actions/checkout@v4\n'+
      '      - name: Run command\n        run: |\n          ${{ github.event.inputs.cmd }}\n';
    await fsPut('.github/workflows/agents-task.yml',yml); refreshDisk();
    tool('✓ Creado .github/workflows/agents-task.yml.\nSúbelo al repo con /git push (necesitas token con actions:write).\nDespués podrás ejecutar comandos REALES de Linux con:\n  /action exec "make test"');
    flowAdd('tool','/action setup',yml.slice(0,300));
    return;
  }
  if(sub==='exec'){
    const cmdrest=rest.trim();
    if(!cmdrest){ tool('Uso: /action exec "<comando>"   — ej: /action exec "make test"\n(requiere /action setup y haber hecho /git push)'); return; }
    setWorking(true);
    try{
      const repo2=repo, H2=ghHeaders();
      const lw=await (await fetch('https://api.github.com/repos/'+repo2+'/actions/workflows',{headers:H2})).json();
      const w=(lw.workflows||[]).find(w=>w.path.split('/').pop()==='agents-task.yml');
      if(!w){ tool('/action exec: falta .github/workflows/agents-task.yml. Ejecuta antes /action setup y /git push.'); return; }
      const ref=await actionDefaultBranch(repo2,H2);
      const dr=await fetch('https://api.github.com/repos/'+repo2+'/actions/workflows/'+w.id+'/dispatches',
        {method:'POST',headers:Object.assign({'Content-Type':'application/json'},H2),body:JSON.stringify({ref:ref,inputs:{cmd:cmdrest}})});
      if(dr.status!==204){ tool('/action exec: HTTP '+dr.status+(dr.status===403?' (token sin permiso actions:write o workflow no subido)':'')); return; }
      tool('▶ Ejecutando en la nube: '+cmdrest);
      const concl=await actionWaitRunById(repo2,'Agents Task',ref,H2);
      if(concl==='success'){
        const log=await actionFetchLog(repo2,H2);
        tool(log? ('Salida del run:\n'+log) : '(sin salida capturada)');
      } else if(concl){ tool('El comando falló en la nube. Revisa la salida arriba o el run en GitHub.'); }
    }catch(e){ tool('/action exec error: '+e); } finally{ setWorking(false); }
    return;
  }
  tool('Uso: /action list | runs | run <workflow.yml|nombre> [rama] | setup | exec "<cmd>"\n'+
       'Ejemplos:\n'+
       '  /action list                    — workflows del repo actual\n'+
       '  /action run pages-build-deployment\n'+
       '  /action run build.yml main      — dispara y sigue el progreso\n'+
       '  /action runs                    — últimos 8 runs con estado\n'+
       '  /action setup                   — crea el workflow de ejecución real (una vez)\n'+
       '  /action exec "make test"        — ejecuta un comando REAL de Linux en la nube');
}
async function actionDefaultBranch(repo,H){
  try{ const j=await (await fetch('https://api.github.com/repos/'+repo,{headers:H})).json(); return j.default_branch||'main'; }catch(e){ return 'main'; }
}
// like actionWaitRun but only accepts runs created after the call (dispatch may race with older runs)
async function actionWaitRunById(repo,wfName,ref,H){
  const t0=Date.now()-30000; let seenId=null;
  for(let i=0;i<36;i++){
    await new Promise(r=>setTimeout(r,i?5000:2000));
    let j=null; try{ j=await (await fetch('https://api.github.com/repos/'+repo+'/actions/runs?per_page=5',{headers:H})).json(); }catch(e){}
    const run=((j&&j.workflow_runs)||[]).find(x=>x.name===wfName&&x.head_branch===ref&&(new Date(x.created_at).getTime()>=t0));
    if(!run || run.id===seenId) continue;
    seenId=run.id;
    if(run.conclusion){
      tool((run.conclusion==='success'?'✓ Run #'+run.run_number+' completado':'✕ Run #'+run.run_number+': '+run.conclusion)+(run.html_url?' → '+run.html_url:''));
      return run.conclusion;
    }
    tool('⏳ Run #'+(run.run_number||'?')+' en curso…');
  }
  tool('/action: timeout esperando el run. Mira el estado con /action runs.');
  return null;
}
// tail of the latest run log (requires token; public repos need auth for logs)
async function actionFetchLog(repo,H){
  try{
    const j=await (await fetch('https://api.github.com/repos/'+repo+'/actions/runs?per_page=1',{headers:H})).json();
    const run=(j.workflow_runs||[])[0]; if(!run) return '';
    const jj=await (await fetch(run.jobs_url,{headers:H})).json();
    const job=(jj.jobs||[])[0]; if(!job) return '';
    const lr=await fetch('https://api.github.com/repos/'+repo+'/actions/jobs/'+job.id+'/logs',{headers:H});
    if(!lr.ok) return '';
    const lines=(await lr.text()).split(/\r?\n/).filter(l=>l.trim());
    return lines.slice(-40).join('\n').slice(0,4000);
  }catch(e){ return ''; }
}
async function actionWaitRun(repo,wfName,ref,H){
  let seenId=null;
  for(let i=0;i<24;i++){
    await new Promise(r=>setTimeout(r,i?5000:2000));
    let j=null; try{ j=await (await fetch('https://api.github.com/repos/'+repo+'/actions/runs?per_page=5',{headers:H})).json(); }catch(e){}
    const run=((j&&j.workflow_runs)||[]).find(x=>x.name===wfName&&x.head_branch===ref);
    if(!run || run.id===seenId) continue;
    seenId=run.id;
    if(run.conclusion){
      tool((run.conclusion==='success'?'✓ Run #'+run.run_number+' completado':'✕ Run #'+run.run_number+': '+run.conclusion)+(run.html_url?' → '+run.html_url:''));
      flowAdd('tool','/action '+run.conclusion,'');
      return run.conclusion;
    }
    tool('⏳ Run #'+(run.run_number||'?')+' en curso…');
  }
...
/* ===== /cron — tareas programadas (local y nube) ===== */
const cronJobs=(()=>{ try{ return JSON.parse(localStorage.getItem('crons')||'[]'); }catch(e){ return []; } })();
function cronSave(){ try{ localStorage.setItem('crons',JSON.stringify(cronJobs)); }catch(e){} }
function cronParseEvery(s){ const m=/^(\d+)(m|h|d)$/i.exec(s||''); if(!m) return 0;
  return parseInt(m[1])*({m:60e3,h:3600e3,d:86400e3})[m[2].toLowerCase()]; }
async function cronCmd(arg){
  arg=(arg||'').trim();
  if(arg==='gh'||arg.startsWith('gh ')){
    const m=/^gh\s+"([^"]+)"\s+(.+)$/.exec(arg);
    if(!m){ tool('Uso: /cron gh "*/15 * * * *" <descripción de la tarea>'); return; }
    const sched=m[1], task=m[2];
    if(!/^\S+\s+\S+\s+\S+\s+\S+\s+\S+$/.test(sched)){ tool('/cron: expresión cron inválida (5 campos): '+sched); return; }
    const yml='name: Agents Cron\n'+
      'on:\n  schedule:\n    - cron: "'+sched+'"\n  workflow_dispatch: {}\n'+
      'jobs:\n  task:\n    runs-on: ubuntu-latest\n    steps:\n'+
      '      - uses: actions/checkout@v4\n'+
      '      - name: Tarea programada (edita este paso)\n        run: |\n'+
      '          echo "Tarea: '+task.replace(/"/g,'\\"')+'"\n'+
      '          # Edita este paso con lo que quieras ejecutar en la nube\n';
    await fsPut('.github/workflows/agents-cron.yml',yml); refreshDisk();
    tool('✓ Creado .github/workflows/agents-cron.yml con schedule "'+sched+'".\nEdita el paso "Tarea programada" (/edit .github/workflows/agents-cron.yml … o sh cat) y sube los cambios con /git push. Se disparará en la nube aunque cierres el navegador.\nCompruébalo después con /action runs.');
    flowAdd('tool','/cron gh '+sched,yml.slice(0,300));
    return;
  }
  if(/^del\s+\d+$/i.test(arg)){
    const id=parseInt(arg.split(/\s+/)[1]); const ix=cronJobs.findIndex(j=>j.id===id);
    if(ix<0){ tool('/cron: no existe la tarea #'+id+'. Usa /cron para listar.'); return; }
    const removed=cronJobs.splice(ix,1)[0]; cronSave();
    tool('✕ Tarea #'+id+' eliminada ('+removed.everyLabel+' · '+removed.cmd+').'); return;
  }
  if(!arg || arg==='list'){
    tool(cronJobs.length?
      cronJobs.map(j=>'#'+j.id+' · cada '+j.everyLabel+' · '+j.cmd).join('\n')+'\n\n/cron del <id> elimina · /cron gh "<schedule>" <tarea> crea un workflow en GitHub Actions.' :
      'Sin tareas programadas.\nUso:\n  /cron <30m|2h|1d> <comando>   — repite un comando mientras esta pestaña esté abierta (ej: /cron 30m /action runs)\n  /cron del <id>                — elimina una tarea\n  /cron gh "*/15 * * * *" <tarea> — programa en la nube vía GitHub Actions');
    return;
  }
  const sp=arg.indexOf(' ');
...
setInterval(()=>{
  ...
  for(const j of cronJobs){
    ...
    if(workN>0){ j.next=now+15000; changed=true; continue; }   // agent busy: retry soon
    j.next=now+j.ms; changed=true;
    tool('⏰ /cron ejecuta: '+j.cmd);
    runCmd(j.cmd);
  }
  if(changed) cronSave();
}, 20000);
...
/* =====================================================================
   Mini command shell + optional LLM agent (OpenCode Zen free models, function calling).
...
/* ===== slash command autocomplete: dropdown under the prompt ===== */
const SLASH_CMDS=[
  ['/help','Muestra la ayuda con todos los comandos','Show help with all commands'],
  ['/clear','Limpia la conversación y el disco de trabajo','Clear the conversation and flow'],
  ['/cost','Métricas de la sesión (tokens, coste)','Session metrics (tokens, cost)'],
  ['/compact','Compacta el contexto de la conversación','Compact the conversation context'],
  ['/init','Crea AGENTS.md con reglas del proyecto','Create AGENTS.md with project rules'],
  ['/key','Información sobre la API key (no se necesita)','Info about API key (not needed)'],
  ['/ghtoken','Fija el token de GitHub para push/pull','Set GitHub token for push/pull'],
  ['/goal','Fija o muestra el objetivo actual','Set or show current goal'],
  ['/plan','Genera un plan en pasos (opcional: tarea)','Generate a step-by-step plan'],
  ['/run','Ejecuta el plan actual','Run the current plan'],
  ['/loop','Bucle autónomo hacia un objetivo','Autonomous loop toward a goal'],
  ['/proxy','Fija el proxy CORS para git/binarios','Set CORS proxy for git/binaries'],
  ['/clone','Clona un repo de GitHub en el disco','Clone a GitHub repo into the disk'],
  ['/git','Operación git: status, add, commit, push…','Git operation: status, add, commit, push…'],
  ...
  ['/sh','Ejecuta un comando de shell simulado','Run a simulated shell command'],
  ['/btw','Interjección: nota para el agente en marcha','Aside: note for the running agent'],
...
function acRender(){
  ...
  const i=document.getElementById('prompt');
  const parts=i.value.slice(1).split(/\s+/);
  const typed='/'+(parts[0]||'').toLowerCase();
  acItems=SLASH_CMDS.filter(c=>c[0].trim().startsWith(typed));
  if(!acItems.length){ acClose(); return; }
  d.innerHTML=acItems.map((c,n)=>
    '<div class="ac-item" data-n="'+n+'" style="display:flex;align-items:center;gap:12px;padding:7px 14px;cursor:pointer;font-size:13px;"'+
    ' onmousedown="event.preventDefault();acPick('+n+')">'+
    '<span style="font-family:ui-monospace,monospace;color:#93c5fd;">'+c[0].trim()+'</span>'+
    '<span style="margin-left:auto;padding-left:16px;text-align:right;color:#9ca3af;font-size:11px;">'+T(c[2],c[1])+'</span></div>').join('');
  Array.from(d.children).forEach(el2=>{
    el2.onmouseenter=()=>{ acIdx=parseInt(el2.dataset.n); acHl(); };
    el2.onmouseleave=()=>{ acIdx=-1; acHl(); };
  });
  d.style.display='block'; acIdx=-1; acHl(); acPlace();
}
function acHl(){ const d=document.getElementById('acdd'); if(!d) return;
  Array.from(d.children).forEach((el2,n)=>{ el2.style.background=(n===acIdx)?'#1f2937':'transparent'; });
  if(acIdx>=0 && d.children[acIdx]) d.children[acIdx].scrollIntoView({block:'nearest'}); }
function acUpdate(){
  const i=document.getElementById('prompt');
  if(!i || !/^\/\S*$/.test(i.value)){ acClose(); return; }   // only while typing "/cmd" (no args yet)
  acRender();
}
function acMove(step){ if(!acItems.length) return; acIdx=(acIdx+step+acItems.length)%acItems.length; acHl(); }
function acPick(n){
  const c=acItems[n]; if(!c) return;
  const i=document.getElementById('prompt');
  i.value=c[0].trim()+' '; acClose(); i.focus();
}
document.addEventListener('mousedown',e=>{ const d=document.getElementById('acdd'); if(d && !d.contains(e.target) && e.target.id!=='prompt') acClose(); });
window.addEventListener('resize',()=>{ if(acOpen()) acPlace(); });

function promptKey(e){ const i=e.target;
  if(e.key==='Escape' && acOpen()){ acClose(); e.preventDefault(); return; }
  if(acOpen()){
    if(e.key==='ArrowDown'){ acMove(1); e.preventDefault(); return; }
    if(e.key==='ArrowUp'){ acMove(-1); e.preventDefault(); return; }
    if(e.key==='Tab'){ acPick(acIdx>=0?acIdx:0); e.preventDefault(); return; }
    if(e.key==='Enter'){ if(acIdx>=0){ acPick(acIdx); } else { acClose(); run(); } e.preventDefault(); return; }
  }
...
async function runDemoLLM(){
  ...
  // ask the LLM for a fresh topic so every demo differs
  let topic='organizar notas de un proyecto';
  try{
    const r=await ocChat({model:MODEL,messages:[{role:'user',content:'Give ONE short idea (max 8 words, in '+(LANGS[curLang]||'español')+') of a small distinct task to demo an agent that creates and edits files on a disk. Only the idea, no quotes or trailing punctuation.'}]});
...
const TOOLS=[
 ...
 {type:'function',function:{name:'classify_disk',description:'Auto-classify all files on the disk using AI (transformers.js zero-shot). Returns a table of file → category. Categories: codigo, test, configuracion, documentacion, datos, script, otro. Useful to understand the project structure without reading every file.',parameters:{type:'object',properties:{}}}}
];
...
async function execToolRaw(name,args){ try{ args=JSON.parse(args||'{}'); }catch(e){ args={}; }
  // permission gate: per-tool persistent policy (allow|ask|deny)
  const gate={write_file:[args.path||''],delete_file:[args.path||''],shell:[args.command||''],python:['código'],cc:['código'],web_fetch:[args.url||''],dispatch_agents:[((args.tasks||[]).length)+' subtareas'],git:[args.action||'']}[name];
  if(gate && !(await permGate(name,gate[0]))) return 'Permiso denegado por el usuario (política: '+permPolicy(name)+').';
  ...
  if(name==='write_file'){ const p=shResolve(args.path);
    ...
    return 'wrote '+p; }
  if(name==='delete_file'){ const p=shResolve(args.path);
  ...
  if(name==='web_search'){ return await webSearchTool(args.query||''); }
  ...
  if(name==='register_tool'){
    ...
    if(!n||n.length<2) return 'Error: nombre de tool invalido (min 2 chars, a-z0-9_)';
    const sp=args.scriptPath||'';
    const f=await fsGet(shResolve(sp));
    if(!f) return 'Error: script no encontrado: '+sp;
    USER_TOOLS.set(n,{name:n,desc:args.description||'',path:sp,type:sp.endsWith('.py')?'python':'shell'});
    saveUserTools();
    return 'Tool registrada: '+n+' ('+USER_TOOLS.get(n).type+'). Usala como comando shell o tool del agente.';
  }
  if(name==='user_tools'){
  ...
  if(name==='classify_disk'){
    classifyCard();
    return 'Clasificación del disco iniciada.';
  }
  ...
  if(name==='create_skill'){
    ...
    await fsPut(sp,sc);
    refreshDisk();
    return 'Skill '+sn+' '+(old?'actualizado':'creado')+'. Archivo: '+sp+'. Usa toggle_skill para activarlo.';
  }
  if(name==='toggle_skill'){
    const sn=(args.name||'').toLowerCase();
    if(!sn) return 'Error: nombre requerido';
    if(!(await skillPrompt(sn))) return 'Error: skill '+sn+' no existe. Usa create_skill primero.';
    const activate=args.active!==false;
    if(activate && !skillsOn.has(sn)){ toggleSkill(sn); return 'Skill '+sn+' activado.'; }
    if(!activate && skillsOn.has(sn)){ toggleSkill(sn); return 'Skill '+sn+' desactivado (archivo conservado).'; }
    return 'Skill '+sn+' ya estaba '+(activate?'activado':'desactivado')+'.';
  }
...
function getKey(){ return 'public'; }   // OpenCode Zen free models need no API key

/* ===== Web search / fetch via fivetechsoft.com proxy (Jina anonymous is blocked) ===== */
const WEBPROXY='https://api.fivetechsoft.com/webfetch.php?url=';
// web_search: Wikipedia API (ES + EN) through the outbound fetch proxy, like termux
async function webSearchTool(query){
  const q=encodeURIComponent(query);
  let out='';
  for(const lang of ['es','en']){
    try{
      const u='https://'+lang+'.wikipedia.org/w/api.php?action=query&list=search&format=json&srlimit=5&srsearch='+q;
      const r=await fetch(WEBPROXY+encodeURIComponent(u));
      if(!r.ok) continue;
      const j=await r.json();
      const hits=((j.query&&j.query.search)||[]).map(s=>'['+lang+'] '+s.title+'\nhttps://'+lang+'.wikipedia.org/wiki/'+encodeURIComponent(s.title.replace(/ /g,'_'))+'\n'+(s.snippet||'').replace(/<[^>]+>/g,''));
      if(hits.length) out+=(out?'\n':'')+hits.join('\n\n');
    }catch(e){}
  }
  return out ? out.slice(0,4000) : 'web_search: sin resultados para "'+query+'"';
}

/* ===== OpenCode Zen: dynamic free-model discovery + fallback ===== */
...
async function discoverFreeModels(){
  ...
  const PREFERRED=['x-preview-f-free','mimo-v2.5-free'];   // Ox Alpha, Mimo
  try{
    const r=await fetch(OC_URL+'/models',{headers:{'Authorization':'Bearer public'}});
    const j=await r.json();
    const free=((j&&j.data)||[]).map(m=>m.id).filter(id=>/-free$/.test(id));
    FREE_MODELS=free.length ? [...PREFERRED.filter(id=>free.includes(id)),...free.filter(id=>!PREFERRED.includes(id))] : PREFERRED;
  }catch(e){ FREE_MODELS=PREFERRED.slice(); }
  return FREE_MODELS;
}
// POST to OpenCode Zen chat/completions: tries MODEL first, then the other
// free models by availability. Returns the raw Response of the first OK call.
async function ocChat(body,opt){
  opt=opt||{};
  const models=[MODEL,...(await discoverFreeModels()).filter(m=>m!==MODEL)];
  let lastErr;
  for(const m of models){
    try{
      body.model=m;
      const r=await fetch(OC_URL+'/chat/completions',{method:'POST',
        headers:{'Content-Type':'application/json','Authorization':'Bearer public'},
        signal:opt.signal, body:JSON.stringify(body)});
      if(r.ok){ if(MODEL!==m){ MODEL=m; try{localStorage.setItem('model',m);}catch(e){} } setModelTag(m); return r; }
      try{ await r.body&&r.body.cancel(); }catch(e){}
      lastErr=new Error('LLM '+r.status);
...
/* ===== puerta de permisos: política allow|ask|deny persistente por herramienta ===== */
const PERM=(()=>{ try{ return JSON.parse(localStorage.getItem('perm')||'{}'); }catch(e){ return {}; } })();
function permSave(){ try{ localStorage.setItem('perm',JSON.stringify(PERM)); }catch(e){} }
const PERM_DEF={write_file:'ask',delete_file:'ask',shell:'ask',cc:'ask',git:'ask',python:'allow',web_search:'allow',web_fetch:'allow',dispatch_agents:'allow'};
function permPolicy(t){ return PERM[t]||PERM_DEF[t]||'allow'; }
async function permGate(t,detail){
  const pol=permPolicy(t);
  if(pol==='allow') return true;
  if(pol==='deny'){ tool(T('⛔ Permiso denegado para "'+t+'" (/perm para cambiarlo).','Permission denied for "'+t+'" (use /perm to change).')); return false; }
  return await askPermit(T('usar la herramienta '+t,'use the '+t+' tool'),detail,t);
}
async function permCmd(arg){
  arg=(arg||'').trim();
  const m=/^(\w+)\s+(allow|ask|deny)$/i.exec(arg);
  if(m){ PERM[m[1]]=m[2].toLowerCase(); permSave();
    tool('Permiso '+m[1]+' = '+m[2].toLowerCase()); return; }
  const keys=[...new Set([...Object.keys(PERM_DEF),...Object.keys(PERM)])];
  tool(keys.map(k=>k+' = '+(PERM[k]||PERM_DEF[k])+(PERM[k]?' *':'')).join('\n')+
    '\n\nUso: /perm <tool> <allow|ask|deny>   (* = personalizado)\nPor defecto, las herramientas que modifican datos piden confirmación.');
}
function askPermit(action,detail,toolKey){ if(autoApprove) return Promise.resolve(true);
  if(toolKey && permPolicy(toolKey)==='allow') return Promise.resolve(true);
  return new Promise(res=>{
    const card=el('<div class="bg-[#2a1f0c] border border-yellow-700/50 p-3 rounded-lg max-w-[90%]"></div>');
    ...
    ok.onclick=()=>{ dis(); flowAdd('permit',action+': '+detail,'✓ permitido'); res(true); }; al.onclick=()=>{ if(toolKey){ PERM[toolKey]='allow'; permSave(); } else { autoApprove=true; } dis(); flowAdd('permit',action+': '+detail,'✓ siempre ('+(toolKey||'todo')+')'); res(true); }; no.onclick=()=>{ dis(); flowAdd('permit',action+': '+detail,'✗ rechazado'); res(false); };
...
let MODEL=(()=>{ try{ return localStorage.getItem('model')||'x-preview-f-free'; }catch(e){ return 'x-preview-f-free'; } })();
setModelTag(MODEL);
// voice: restore TTS button state + warm up the voices list
(function(){ if(TTS_ON){ const b=document.getElementById('ttsbtn'); if(b) b.classList.add('bg-emerald-700'); }
  try{ if(window.speechSynthesis){ speechSynthesis.getVoices(); speechSynthesis.onvoiceschanged=()=>speechSynthesis.getVoices(); } }catch(e){} })();
// resolve the friendly name as soon as the free-model list is available
discoverFreeModels();
...
async function streamChat(msgs, wait){
  ...
  const r=await ocChat(body,{signal:agentAbort.signal});
...
async function agent(text){
  ...
  try{
    ...
    while(true){
      ...
      let wait=bot('<span class="text-gray-400">pensando…</span>',true); let res;
      try{ res=await streamChat(msgs, wait); }
        ...
        wait.remove(); convo.pop(); errorCard('Error de conexión','No se pudo contactar con el LLM (red o CORS).',String(e),()=>agent(text)); return; }   // pop: agent(text) re-pushes it on retry
      addUsage(res.usage);
...
async function shareSession(){
  ...
  // safety net: if the GitHub token ever leaked into the conversation
  // or a disk file, redact it before the session leaves this browser
  try{ const secrets=[ghTok()||''].filter(s=>s.length>8);
...
async function slashCmd(v){
  ...
  switch(cmd){
    ...
    case '/action': await actionCmd(arg); break;
...
// OpenCode Zen free models: no billing — session cost is $0.00 (tokens still tracked).
function estCost(u){ return 0; }
function costCard(u){
...
function skillCard(skills){
  ...
  skills.forEach(s=>{ const on=skillsOn.has(s.name); const isDisk=s.disk;
    ...
    const iconColor=isDisk?(on?'text-purple-400':'text-gray-500'):(on?'text-pink-400':'text-gray-500');
    ...
    nm.innerHTML=(isDisk?'<span class=\"text-purple-400\">📄</span> ':'')+s.name+(isDisk?' <span class=\"text-[8px] text-purple-400 bg-purple-900/40 px-1 rounded\">disco</span>':' <span class=\"text-[8px] text-pink-400 bg-pink-900/40 px-1 rounded\">built-in</span>');
    ...
    const tg=toggleEl(on,()=>{ toggleSkill(s.name); const nv=skillsOn.has(s.name); setToggle(tg,nv); row.classList.toggle('opacity-60',!nv); const nc=isDisk?(nv?'text-purple-400':'text-gray-500'):(nv?'text-pink-400':'text-gray-500'); ic.className='mt-0.5 '+nc; nm.className='text-xs font-medium '+(nv?'text-gray-200':'text-gray-400'); ds.className='text-[10px] mt-0.5 leading-tight pr-2 '+(nv?'text-gray-400':'text-gray-500'); });
...
async function skillCmd(arg){
  ...
  if(sub==='on'){
    if(!rest){ tool('Uso: /skill on <nombre> — activa un skill'); return; }
    if(!(await skillPrompt(rest))){ tool('Skill '+rest+' no existe. Usa /skill new '+rest+' para crearlo.'); return; }
    if(skillsOn.has(rest)){ tool('Skill '+rest+' ya estaba activo.'); return; }
    toggleSkill(rest); skillCard(await listSkills());
    return;
  }
  if(sub==='off'){
    if(!rest){ tool('Uso: /skill off <nombre> — desactiva un skill (no lo borra)'); return; }
    if(!skillsOn.has(rest)){ tool('Skill '+rest+' ya estaba inactivo.'); return; }
    toggleSkill(rest); skillCard(await listSkills());
    return;
  }
  if(sub==='generate'){
...
async function planReal(task){
  ...
  const wait=bot('<span class="text-gray-400">generando plan…</span>',true); agentAbort=new AbortController();
  setWorking(true);
  try{
  ...
  }catch(e){ if(e.name==='AbortError') wait.innerHTML='(detenido)'; else wait.innerHTML='Error: '+e+' (posible CORS)'; }
  finally{ setWorking(false); }
}
